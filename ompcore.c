...
/*mwIndex is a type that represents index values, such as indices into arrays. */
/*    Use this function for cross-platform flexibility. */
/* By default, mwIndex is equivalent to size_t in C. */
                 ...
                 int T, double eps, int gamma_mode, int profile, double msg_delta, int erroromp)
{
  ...
  if (gamma_mode == FULL_GAMMA) {
    ...
    gammaPr = mxGetPr(Gamma); /*Real data elements in mxDOUBLE_CLASS array. Use mxGetPr on real arrays of type mxDOUBLE_CLASS only.*/
    
    gammaIr = mxGetIr(Gamma); /*Use mxGetIr to obtain the starting address of the ir array. 
     * The ir array is an array of integers.*/
    gammaJc = mxGetJc(Gamma); /*Use mxGetJc to obtain the starting address of the jc array. 
     * The jc array is an integer array having n+1 elements, where n is the number of columns in the sparse mxArray.
     * The values in the jc array indirectly indicate columns containing nonzero elements. */
    gamma_count = 0;
    gammaJc[0] = 0;
  }
